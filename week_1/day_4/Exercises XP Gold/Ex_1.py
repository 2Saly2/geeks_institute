...
class MinimumBalanceAccount(BankAccount):
    ...
    def withdraw(self, amount):
       ...
       if self.balance - amount < self.minimum_balance :
           raise Exception("Cannot withdraw, minimum balance limit reached.")
       self.balance -= amount 
       print(f"Withdrew {amount}. New balance: {self.balance}")
...
class ATM:
    def __init__(self,account_list , try_limit=2):
        ...
        if not isinstance(try_limit, int) or try_limit <= 0:
            print("Invalid try_limit, setting it to 2 by default.")
            try_limit = 2
...
if __name__ == "__main__":
    acc1 = BankAccount("nouhaila", "1234", 500)
    acc2 = MinimumBalanceAccount("alli", "pass", 1000, minimum_balance=200)
